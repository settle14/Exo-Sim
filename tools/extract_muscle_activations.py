...
def rollout_and_export(agent, xml_path, out_dir):
    ...
    TMAX = 100000
    t_list, act_list = [], []
    exo_tau_list = []  # EXO 力矩（ctrl）
    r_m_list, work_proxy_list = [], []  # 辅助效果参数
    ...
    while True:
        ...
        # 提取辅助效果相关参数 (r_m, work_proxy)
        # r_m: 肌肉激活的均值（全身肌肉）
        r_m = np.mean(ctrl[mus_ids]) if len(mus_ids) > 0 else 0.0
        
        # work_proxy: 肌肉激活的平方和作为功的代理指标
        work_proxy = np.sum(ctrl[mus_ids] ** 2) if len(mus_ids) > 0 else 0.0
        ...
        r_m_list.append(r_m)
        work_proxy_list.append(work_proxy)
    ...
    if len(act_list) == 0:
        ...
        r_m_arr = np.zeros(1, np.float32)
        work_proxy_arr = np.zeros(1, np.float32)
    else:
        acts = np.vstack(act_list)           # [T, M]
        r_m_arr = np.asarray(r_m_list, np.float32)
        work_proxy_arr = np.asarray(work_proxy_list, np.float32)
    ...
    def save_scalar_csv(path, scalar_data, data_name):
        with open(path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["time", data_name])
            for i in range(len(t)):
                w.writerow([f"{t[i]:.6f}", f"{scalar_data[i]:.6f}"])

    # 保存文件
    act_csv = os.path.join(out_dir, "muscle_activations.csv")
    r_m_csv = os.path.join(out_dir, "r_m_timeseries.csv")
    work_proxy_csv = os.path.join(out_dir, "work_proxy_timeseries.csv")
    exo_tau_csv = os.path.join(out_dir, "exo_torque.csv")

    save_csv(act_csv, acts, mus_names)
    save_scalar_csv(r_m_csv, r_m_arr, "r_m")
    save_scalar_csv(work_proxy_csv, work_proxy_arr, "work_proxy")
    ...
    meta = dict(
        ...
        fields=dict(
            ...
            r_m="mean muscle activation (AU)",
            work_proxy="sum of squared muscle activations (AU^2)"
        )
    )
    ...
    print(" - r_m_timeseries.csv")
    print(" - work_proxy_timeseries.csv")
    if exo_tau is not None:
...
def main():
    ...
    ap.add_argument("--epoch", type=int, default=-1, help="Checkpoint epoch to load (-1 for latest)")
    ...
    ap.add_argument("--exo_actuators", type=str, default="exo_knee_r,exo_knee_l,exo_hip_r,exo_hip_l",
